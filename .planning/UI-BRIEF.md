# UI Design Brief — ChittiApp

**Use this file as a paste-ready prompt for claude.ai.** Paste the body between the `─── PROMPT START ───` and `─── PROMPT END ───` markers into a fresh Claude conversation; ask for HTML/React artifact mockups, one screen at a time or in a single tabbed artifact.
...
- **Tightening:** If Claude over-produces, trim Screens 6 (Member view) into Screen 5 with a "switch role" toggle. The 9-screen version still covers all the product moments.
- **Expanding:** Add Screen 11 — "Audit log" — when ready to design the dispute-resolution moment.
- **Brand iteration:** If the first palette doesn't land, reply with concrete pulls — "make it more like Splitwise / less like Cred" — and ask Claude to re-propose in 30 seconds.
- **Implementation handoff:** When ready, ask Claude to convert designs to React Native + Tailwind RN (NativeWind) or your styling primitive of choice. The design language above (rounded, soft shadows, generous whitespace) translates cleanly.

---
*Brief created: 2026-05-22 — paste body between PROMPT markers into a fresh claude.ai conversation.*
